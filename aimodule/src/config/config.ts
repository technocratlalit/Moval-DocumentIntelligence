...
const {
  ...
  MISTRAL_OCR_INCLUDE_BLOCKS,
  MISTRAL_OCR_EXTRACT_HEADER,
  MISTRAL_OCR_EXTRACT_FOOTER,
  MISTRAL_OCR_CONFIDENCE,
  MISTRAL_OCR_INCLUDE_IMAGE_BASE64,
  MISTRAL_OCR_MAX_PAGES,
  MISTRAL_COST_PER_PAGE_USD,
  CLAIM_AI_MODEL,
  CLAIM_CLASSIFY_AI_MODEL,
  CLAIM_MAX_OUTPUT_TOKENS,
  CLAIM_MAX_GEMINI_RETRIES,
  CLAIM_DEBUG_DUMP,
  CLAIM_DEBUG_DUMP_DIR,
  CLAIM_DEBUG_MAX_CHARS,
} = process.env;
...
export const _config = {
  ...
  // Mistral OCR (claim Pass 1)
  MISTRAL_API_KEY,
  MISTRAL_OCR_MODEL: parseEnvOptional(MISTRAL_OCR_MODEL) ?? 'mistral-ocr-latest',
  MISTRAL_OCR_TABLE_FORMAT: parseEnvOptional(MISTRAL_OCR_TABLE_FORMAT) ?? 'html',
  MISTRAL_OCR_INCLUDE_BLOCKS: parseEnvBool(MISTRAL_OCR_INCLUDE_BLOCKS, true),
  MISTRAL_OCR_EXTRACT_HEADER: parseEnvBool(MISTRAL_OCR_EXTRACT_HEADER, false),
  MISTRAL_OCR_EXTRACT_FOOTER: parseEnvBool(MISTRAL_OCR_EXTRACT_FOOTER, false),
  MISTRAL_OCR_CONFIDENCE: parseEnvOptional(MISTRAL_OCR_CONFIDENCE) ?? 'word',
  MISTRAL_OCR_INCLUDE_IMAGE_BASE64: parseEnvBool(MISTRAL_OCR_INCLUDE_IMAGE_BASE64, false),
  MISTRAL_OCR_MAX_PAGES: parseEnvInt(MISTRAL_OCR_MAX_PAGES, 10),
  MISTRAL_COST_PER_PAGE_USD: parseEnvFloat(MISTRAL_COST_PER_PAGE_USD, 0.002),

  // Claim Pass 2+ (Gemini classify + map)
  CLAIM_AI_MODEL: parseEnvOptional(CLAIM_AI_MODEL) ?? resolvedAiModelLite,
  CLAIM_CLASSIFY_AI_MODEL: parseEnvOptional(CLAIM_CLASSIFY_AI_MODEL) ?? resolvedAiModelLite,
  ...
  CLAIM_DEBUG_DUMP: parseEnvBool(CLAIM_DEBUG_DUMP, false),
  CLAIM_DEBUG_DUMP_DIR: parseEnvOptional(CLAIM_DEBUG_DUMP_DIR),
  CLAIM_DEBUG_MAX_CHARS: parseEnvInt(CLAIM_DEBUG_MAX_CHARS, 12_000),
